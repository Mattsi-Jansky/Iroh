...
pub fn generate_bishop_moves(available_moves: &mut Vec<Move>, bishop: (Tile, Coordinate), game_state: &GameState, is_for_first_player: bool) {
    generate_dynamic_moves(game_state, available_moves, bishop, &DIAGONAL_DYNAMIC_TRANSFORMERS, is_for_first_player);
}

pub fn generate_rook_moves(available_moves: &mut Vec<Move>, rook: (Tile, Coordinate), game_state: &GameState, is_for_first_player: bool) {
    generate_dynamic_moves(game_state, available_moves, rook, &STRAIGHT_DYNAMIC_TRANSFORMERS, is_for_first_player);
}

fn generate_dynamic_moves(game_state: &GameState, available_moves: &mut Vec<Move>, origin: (Tile, Coordinate), transformers: &[fn(Coordinate) -> Option<Coordinate>], is_for_first_player: bool) {
